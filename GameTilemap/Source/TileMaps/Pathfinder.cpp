...
Pathfinder::Pathfinder(TileMap* pTilemap)
{
    ...
    m_MapDimensions = m_pTilemap->m_LayoutDimensions;
    m_NumNodes = m_MapDimensions.x * m_MapDimensions.y;
...
{
    assert( tx >= 0 && tx < m_MapDimensions.x &&
            ty >= 0 && ty < m_MapDimensions.y );

    // Calculate the node index based on the tiles x/y.
    return ty*m_MapDimensions.x + tx;
}

int Pathfinder::CheckIfNodeIsClearAndReturnNodeIndex(int tx, int ty)
{
    if( tx < 0 || ty < 0 || tx >= m_MapDimensions.x || ty >= m_MapDimensions.y )
        return -1;
    ...
    if( m_pTilemap->GetIsWalkable( tx, ty ) == false )
        return -1;
...
void Pathfinder::AddNeighboursToOpenList(int nodeIndex, int endNodeIndex)
{
    ...
    int tx = nodeIndex % m_MapDimensions.x;
    int ty = nodeIndex / m_MapDimensions.x;
    ...
    if (!k_AI_DiagonalMovementAllowed)
    {
    ...
    else
    {
        ...
        neighbours[0] = CheckIfNodeIsClearAndReturnNodeIndex(tx, ty + 1); // 10 + width
        neighbours[1] = CheckIfNodeIsClearAndReturnNodeIndex(tx - 1, ty + 1); // 10 + 1
        neighbours[2] = CheckIfNodeIsClearAndReturnNodeIndex(tx - 1, ty); // 10 - width
        neighbours[3] = CheckIfNodeIsClearAndReturnNodeIndex(tx - 1, ty - 1); // 10 - 1
        neighbours[4] = CheckIfNodeIsClearAndReturnNodeIndex(tx, ty - 1); // 10 + width
        neighbours[5] = CheckIfNodeIsClearAndReturnNodeIndex(tx + 1, ty - 1); // 10 + 1
        neighbours[6] = CheckIfNodeIsClearAndReturnNodeIndex(tx + 1, ty); // 10 - width
        neighbours[7] = CheckIfNodeIsClearAndReturnNodeIndex(tx + 1, ty + 1); // 10 - 1
        // Loop through the array.
        for (int i = 0; i < 8; i++)
        {
            int nodeToAdd;
            ...
            if (nodeToAdd != -1)
            {
                ...
                float cost; // Assume a travel cost of 1 for each tile.
                if (i % 2)
                    cost = 1.5;
                else
                    cost = 1;
...
float Pathfinder::CalculateH(int nodeIndex1, int nodeIndex2)
{
    ...
    int tx1 = nodeIndex1 % m_MapDimensions.x;
    int ty1 = nodeIndex1 / m_MapDimensions.x;

    int tx2 = nodeIndex2 % m_MapDimensions.x;
    int ty2 = nodeIndex2 / m_MapDimensions.x;
